...
const getRecentBook = async (bookId) => {
  let [data] = await pool.execute(
    'SELECT owned_books.*, product.* FROM owned_books JOIN product ON owned_books.product_id = product.id WHERE owned_books.member_id = ? AND owned_books.reading_progress <> 0 AND owned_books.update_time IS NOT NULL ORDER BY owned_books.update_time DESC LIMIT 1',
    [bookId]
  );
  return data;
};
